...
@section('add_button')
    <div>
        <a href="{{ route('admin.user.management.reviews.create') }}" class="create-btn btn btn-md mr-3 d-flex justify-content-between align-items-center text-white">
...
@section('content')
    <div class="row mt-3">
        ...
        <div class="card px-3 py-3">
            ...
            <table id="allReviews" class="display" style="width:100%">
                <thead>
                    <tr>
                        <th>Sl</th>
...
@push('scripts')
<script>
    var tables  = $('#allReviews').DataTable({
        processing: true,
        serverSide: true,
        ...
        ajax: {
            url: "{{ route('admin.user.management.reviews.get-data') }}",
            type: "POST",
            dataType: "JSON",
            data: function(d) {
                d._token = _token,
        ...
        buttons: {
            buttons: [{
                ...
                },
                {
                    extend: 'pdf',
                    title: 'All Reviews',
                    filename: 'all_revies_{{ date('d-m-Y') }}',
